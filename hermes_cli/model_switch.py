...
def switch_model(
    ...
    # A managed switch is model-only.  Provider identity, endpoint, and token
    # are regenerated by the enterprise resolver; never inspect local
    # catalogs or carry the current session's runtime bundle forward.
    if enterprise_runtime_required():
        ...
        if requested_provider and requested_provider not in {
            ...
            return ModelSwitchResult(
                ...
                is_global=is_global,
                error_message=(
                    "Enterprise managed policy denied runtime model switch: "
                    f"provider '{requested_provider}' is not the enterprise gateway provider"
                ),
            )
        target_model = raw_input.strip() or enterprise_current_model(load_enterprise_policy())
        try:
        ...
        return ModelSwitchResult(
            ...
            target_provider=ENTERPRISE_PROVIDER,
            provider_changed=current_provider != ENTERPRISE_PROVIDER,
            api_key=runtime.get("api_key", ""),
            base_url=runtime.get("base_url", ""),
            api_mode=runtime.get("api_mode", ""),
            provider_label="Company Gateway",
            is_global=is_global,
        )
