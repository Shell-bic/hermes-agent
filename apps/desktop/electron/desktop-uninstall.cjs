...
function buildWindowsCleanupScript({
  ...
  lines.push(
    'set /a waited=0',
    'set "PROBE_FILE=%TEMP%\\hermes-uninstall-probe-%PID%-%RANDOM%-%RANDOM%.txt"',
    ':waitloop',
    ...
    'rem Keep tasklist separate from findstr: a denied/failed probe is unknown,',
    'rem never proof that the desktop exited.',
    'tasklist /NH /FI "PID eq %PID%" >"%PROBE_FILE%" 2>nul',
    'if %ERRORLEVEL% neq 0 goto wait_retry',
    'findstr /r /c:" %PID% " <"%PROBE_FILE%" >nul',
